/*
 * $RCSfile: SecurityProfileList.java,v $    $Revision: 1.1.1.1 $  $Date: 2005/04/28 20:26:36 $ - $Author: mking_cv $
 * 
...
/*
 * This class is for storing the Security Profile List
 */
	
public class SecurityProfileList extends DisplayList
{

	private String sortMember ;
	private String primaryMember ;
	private String primaryTable ="securityprofile";
	private String PrimaryMemberType="profileid";
	...
	private static Logger logger = Logger.getLogger(SecurityProfileList.class);
	...
	public SecurityProfileList() {
		...
		columnMap.put("ProfileID", new Integer(15));
		columnMap.put("ProfileName", new Integer(15));
		columnMap.put("NoOfUsers", new Integer(15));
		
		this.setPrimaryMember("ProfileID");	
	...
	{	
		if (key != null)
		{
			...
			try
			{
				Authorization remote =(Authorization)authorizationHome.create();
				remote.setDataSource(this.dataSource);
				remote.deleteSecurityProfile(pID,indvID);
			} catch (Exception e)
			{
				logger.error("[Exception] SecurityProfileList.deleteElement( int indvID, String rowId[] )", e);
			}
		}
	}
		...
		try
		{
			...
			for (int i=0; i<recordID.length; i++)
			{
				if(recordID[i] != null && !recordID[i].equals("")){
					...
					try{
						remote.deleteSecurityProfile(elementID,individualID);
					}//end of try block
