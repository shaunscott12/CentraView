/*
 * $RCSfile: PurchaseOrderListHandler.java,v $    $Revision: 1.1.1.1 $  $Date: 2005/04/28 20:26:25 $ - $Author: mking_cv $
 * 
...
public class PurchaseOrderListHandler extends org.apache.struts.action.Action {

	// Global Forwards
	public static final String GLOBAL_FORWARD_failure = "failure";

	// Local Forwards
	private static final String FORWARD_showpurchaseorderlist = "showpurchaseorderlist";
	private static String FORWARD_final = GLOBAL_FORWARD_failure;
	private static Logger logger = Logger.getLogger(PurchaseOrderListHandler.class);

	public PurchaseOrderListHandler() {
    ...
    String dataSource = Settings.getInstance().getSiteInfo(CVUtility.getHostName(super.getServlet().getServletContext())).getDataSource();
		...
		ListPreference listpreference= userobjectd.getListPreference("PurchaseOrder");
		...
		PurchaseOrderList displaylistSession = null;
		PurchaseOrderList displaylist = null;

		try
		{
			displaylistSession = ( PurchaseOrderList )session.getAttribute( "displaylist") ;//gets the list from session
		}
		catch( Exception e )
		{
		...
		try
		{
			displaylist = ( PurchaseOrderList )request.getAttribute( "displaylist") ;//gets the list from request
		}
		catch( Exception e )
		{
			displaylist = null;
		...
		// forward to jsp page
		FORWARD_final = FORWARD_showpurchaseorderlist;
	...
	*	This function sets links on members
	@ 	returns PurchaseOrderList object
	*/
	public PurchaseOrderList setLinksfunction( PurchaseOrderList DL )
	{
		String url = null;
		...
		while( it.hasNext() )
		{
			try
			{
				...
				IntMember intPOID = (IntMember)ele.get("PurchaseOrderID");
				int poID = 0;

				if(intPOID != null)
					poID = ((Integer)intPOID.getMemberValue()).intValue();

				//sets the URL for this member
				intPOID.setRequestURL("goTo('/centraview/DisplayEditPurchaseOrder.do?rowId="+poID+"&"+AccountConstantKeys.TYPEOFOPERATION+"=ShowPurchaseOrder')");
			...
			{
				logger.error("[Exception] PurchaseOrderListHandler.setLinksfunction Handler ", e);
			}
		}
		return DL;
	}
