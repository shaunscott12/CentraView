/*
 * $RCSfile: SaveExpenseHandler.java,v $    $Revision: 1.1.1.1 $  $Date: 2005/04/28 20:26:14 $ - $Author: mking_cv $
 * 
...
public class SaveExpenseHandler  extends org.apache.struts.action.Action
{
	...
	private static Logger logger = Logger.getLogger(SaveExpenseHandler.class);
		...
		throws IOException, ServletException, CommunicationException, NamingException
	{
	  String dataSource = Settings.getInstance().getSiteInfo(CVUtility.getHostName(super.getServlet().getServletContext())).getDataSource();
		...
		try
		{
			...
			if (expenseID > 0)
			{
				remote.updateExpense(expenseVO,individualID);
				expenseVO = remote.getExpenseVO(expenseID,individualID);
			...
			if(typeOfSave != null && typeOfSave.equals("savenew"))
			{
				...
				request.removeAttribute("expenseform");
		...
		{
			logger.error("[Exception] SaveExpenseHandler.Execute Handler ", e);
