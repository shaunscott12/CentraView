/*
 * $RCSfile: RelatedProjectList.java,v $    $Revision: 1.1.1.1 $  $Date: 2005/04/28 20:28:36 $ - $Author: mking_cv $
 * 
 ...
 * screens where the listType is "Project". The class will generate
 * the proper DisplayList and set a request attribute. Control will then
 * transfer to the View layer.
 */
public class RelatedProjectList extends org.apache.struts.action.Action
{
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException
  {
    String dataSource = Settings.getInstance().getSiteInfo(CVUtility.getHostName(super.getServlet().getServletContext())).getDataSource();
    String viewType = "Project";
    ...
    if (listFor.equals("Entity")) {
      filter = "SELECT projectId FROM projectlink WHERE recordTypeId = 14 AND recordId = " + recordID;
    } else if (listFor.equals("Individual")) {
      filter = "SELECT ProjectID FROM projectlink WHERE RecordTypeID=15 AND RecordID=" + recordID;
    }
    // Buttons
    ...
    RelatedInfoUtil.relatedInfoSetup(request, dataSource, viewType, ValueListConstants.PROJECT_LIST_TYPE, ValueListConstants.projectViewMap, filter.toString(), buttonList, 0);
