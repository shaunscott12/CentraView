/*
 * $RCSfile: RelatedOpportunityList.java,v $    $Revision: 1.1.1.1 $  $Date: 2005/04/28 20:28:36 $ - $Author: mking_cv $
 * 
 ...
 */
public class RelatedOpportunityList extends org.apache.struts.action.Action
{
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException
  {
    String dataSource = Settings.getInstance().getSiteInfo(CVUtility.getHostName(super.getServlet().getServletContext())).getDataSource();
    String viewType = "Opportunity";
    ...
    if (listFor.equals("Entity")) {
      filter = "SELECT opportunityId FROM opportunity WHERE entityID = " + recordID;
    } else if (listFor.equals("Individual")) {
      filter = "SELECT o.opportunityId FROM opportunity o WHERE o.IndividualID = " + recordID;
    }
    // Buttons
    ...
    RelatedInfoUtil.relatedInfoSetup(request, dataSource, viewType, ValueListConstants.OPPORTUNITY_LIST_TYPE, ValueListConstants.opportunityViewMap, filter.toString(), buttonList, 0);
