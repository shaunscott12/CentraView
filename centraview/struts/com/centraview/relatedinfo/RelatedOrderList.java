/*
 * $RCSfile: RelatedOrderList.java,v $    $Revision: 1.1.1.1 $  $Date: 2005/04/28 20:28:36 $ - $Author: mking_cv $
 * 
 ...
 * screens where the listType is "Order". The class will generate
 * the proper DisplayList and set a request attribute. Control will then
 * transfer to the View layer.
 */
public class RelatedOrderList extends Action
{
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException
  {
    ...
    if (listFor.equals("Entity")) {
      filter = "SELECT orderid FROM cvorder WHERE entityid=" + recordID;
    } else if (listFor.equals("Project")) {
      filter = "SELECT orderid FROM cvorder WHERE project=" + recordID;
    }
    // Buttons
    ...
    RelatedInfoUtil.relatedInfoSetup(request, dataSource, viewType, ValueListConstants.ORDER_LIST_TYPE, ValueListConstants.orderViewMap, filter.toString(), buttonList, 0);
