/*
 * $RCSfile: RelatedHistoryList.java,v $    $Revision: 1.1.1.1 $  $Date: 2005/04/28 20:28:36 $ - $Author: mking_cv $
 * 
 ...
 */
public class RelatedHistoryList extends org.apache.struts.action.Action
{
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException
  {
    String dataSource = Settings.getInstance().getSiteInfo(CVUtility.getHostName(super.getServlet().getServletContext())).getDataSource();
    String viewType = "History";
    ...
    if (listFor.equals("Entity")) {
      filter = "SELECT hst.historyid historyid FROM history hst, module md  WHERE hst.recordtypeid=md.moduleid and md.name ='" + AdministrationConstantKeys.ENTITY + "' and hst.recordid=" + recordID;
    } else if (listFor.equals("Individual")) {
      filter = "SELECT hst.historyid historyid FROM history hst, module md  WHERE hst.recordtypeid=md.moduleid and md.name ='" + AdministrationConstantKeys.INDIVIDUAL + "' and hst.recordid=" + recordID;
    } else if (listFor.equals("Project")) {
      filter = "SELECT hst.historyid historyid FROM history hst, module md  WHERE hst.recordtypeid=md.moduleid and md.name ='" + AdministrationConstantKeys.PROJECTS + "' and hst.recordid=" + recordID;
    }
    // Buttons
    ...
    RelatedInfoUtil.relatedInfoSetup(request, dataSource, viewType, ValueListConstants.HISTORY_LIST_TYPE, ValueListConstants.historyViewMap, filter.toString(), buttonList, recordID.intValue());
