/*
 * $RCSfile: RelatedExpenseList.java,v $    $Revision: 1.1.1.1 $  $Date: 2005/04/28 20:28:35 $ - $Author: mking_cv $
 * 
 ...
 * screens where the listType is "Expense". The class will generate
 * the proper DisplayList and set a request attribute. Control will then
 * transfer to the View layer.
 */
public class RelatedExpenseList extends Action
{
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException
  {
    String viewType = "Expense";
    ...
    // Filter
    String filter = "";
    if (listFor.equalsIgnoreCase("Opportunity")) {
      filter = "SELECT ExpenseID FROM expense WHERE Opportunity = " + recordId;
    } else if (listFor.equalsIgnoreCase("Project")) {
      filter = "SELECT ExpenseID FROM expense WHERE Project = " + recordId;
    }
    // Buttons
    ...
    RelatedInfoUtil.relatedInfoSetup(request, dataSource, viewType, ValueListConstants.EXPENSES_LIST_TYPE, ValueListConstants.expensesViewMap, filter.toString(), buttonList, 0);
    return mapping.findForward(forwardPage);
  } //end of execute method
} //end of RelatedExpenseList class
