/*
 * $RCSfile: RulesValueListHandler.java,v $    $Revision: 1.1.1.1 $  $Date: 2005/04/28 20:28:08 $ - $Author: mking_cv $
 * 
...
/**
 * The Class is used to process the Email Filtering Rules on incoming messages
 * The EJB layer and pass it to the display layer.
 * @author Naresh Patel <npatel@centraview.com>
 */
public class RulesValueListHandler extends Action {
  private static Logger logger = Logger.getLogger(RulesValueListHandler.class);
  
  public ActionForward execute(ActionMapping mapping,ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException,	CommunicationException,	NamingException
  {
    String final_Forward = ".view.email.rules.list";
    ...
    ListPreference listPreference = userObject.getListPreference("Rule");
    ...
    {
      listParameters = new ValueListParameters( ValueListConstants.RULE_LIST_TYPE,listPreference.getRecordsPerPage(),1);
    ...
    {
      FieldDescriptor sortField = (FieldDescriptor) ValueListConstants.ruleViewMap.get(listPreference.getSortElement());
    ...
    listParameters.setAccountID(accountID.intValue());
    // TODO remove crappy map between old views and new views.
    Vector viewColumns = view.getColumns();
    ArrayList columns = new ArrayList();
    ActionUtil.mapOldView(columns, viewColumns, ValueListConstants.RULE_LIST_TYPE);
    ...
    buttonList.add(new Button("Delete", "delete", "vl_deleteList("+accountID+");", false));
    buttonList.add(new Button("Enable", "enable", "vl_enableList("+accountID+");", false));
    buttonList.add(new Button("Disable", "disable", "vl_disableList("+accountID+");", false));
    ...
    // Stick the list on the request and then the custom list tag will handle it.
    if (logger.isDebugEnabled()) {
    ...
    request.setAttribute("accountID", accountID.toString());
    request.setAttribute("listType", "Rule");
