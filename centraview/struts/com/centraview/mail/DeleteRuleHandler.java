/*
 * $RCSfile: DeleteRuleHandler.java,v $    $Revision: 1.2 $  $Date: 2005/09/07 19:38:09 $ - $Author: mcallist $
 * 
...
/**
 * Class is used to carry out the request to delete the selected rule by user.
 * 
 * @author Naresh Patel <npatel@centraview.com>
 */
public class DeleteRuleHandler extends Action
{
  private static Logger logger = Logger.getLogger(DeleteRuleHandler.class);
    ...
    try
    {
      ...
      for (int i=0; i<rowId.length; i++)
      {
        if(rowId[i] != null && !rowId[i].equals(""))
        {
          ...
          try
          {
          	int numberDeleted = remote.deleteRule(elementID);
