/*
 * $RCSfile: SupportSubReportBuilder.java,v $    $Revision: 1.1.1.1 $  $Date: 2005/04/28 20:22:59 $ - $Author: mking_cv $
 * 
 ...
 * <p>Description: Class for Support standard child report building</p>
...
public class SupportSubReportBuilder extends ReportBuilder{
  ...
  public SupportSubReportBuilder(int reportId,
                                 String ds,
                                 CVDal con
                                 ) throws Exception {
    super(ds,con);
    ...
    if ( reportId != SUPPORT1_REPORT_ID
       ...
       throw new Exception("SupportSubReportBuilder unknown reportId: "+reportId);
    }

    this.reportId = reportId;

    ReportColumn[] colName = new ReportColumn[3];
    colName[0]  = new ReportColumn("UserId",false);
    colName[1]  = new ReportColumn("User",true);
    colName[2]  = new ReportColumn("Ticket Count",true);
    setColumns(colName);

    addPriorSQL("DROP TABLE IF EXISTS repSupport2");
    // bind ArrayList is empty for this sql
    addPriorSQLBind(null);

    // ticket count by user
    addPriorSQL("CREATE TEMPORARY TABLE repSupport2 "+
                "SELECT OwnerId, OwnerName,count(TicketId) AS Count "+
                "FROM repSupport1 GROUP BY OwnerId");
    addPriorSQLBind(null);

    // report content (ticket count by user)
    addReportSQL("SELECT OwnerId,OwnerName, Count FROM repSupport2");
    addReportSQLBind(null);

    addPostSQL("DROP TABLE IF EXISTS repSupport2");
    // bind ArrayList is empty for this sql
    addPostSQLBind(null);
