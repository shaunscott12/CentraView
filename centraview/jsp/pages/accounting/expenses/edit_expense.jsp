<%--
 * $RCSfile: edit_expense.jsp,v $ $Revision: 1.2 $ $Date: 2005/06/14 21:11:42 $ - $Author: mking_cv $
 * 
...
<html:form action="/accounting/view_expense.do">
<html:errors/>
...
function fnItemLookup() {
  ...
  function setItem(itemLookupValues)
  {
	...
	if (flag) {
		document.expenseform.action = "<bean:message key='label.url.root' />/accounting/view_expense.do?<%=AccountConstantKeys.TYPEOFOPERATION%>=<%=AccountConstantKeys.ADDITEM%>";
	} else {
		document.expenseform.action = "<bean:message key='label.url.root' />/accounting/view_expense.do?<%=AccountConstantKeys.TYPEOFOPERATION%>=<%=AccountConstantKeys.REMOVEITEM%>";
	}
...
function saveChanges(act) {
	document.expenseform.action="<bean:message key='label.url.root' />/accounting/save_expense.do?typeOfSave=" + act;
	document.expenseform.submit();
	return false;
}

function saveExpense(typeOfSave) {
	document.expenseform.action="<bean:message key='label.url.root' />/accounting/save_expense.do?typeOfSave=" + typeOfSave;
	document.expenseform.submit();
	return false;
}
