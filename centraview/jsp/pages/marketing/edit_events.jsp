<%
/**
 * $RCSfile: edit_events.jsp,v $    $Revision: 1.3 $  $Date: 2005/08/10 13:31:44 $ - $Author: mcallist $
 *
...
<!--
  ...
  function deleteevent()
  {
    ...
    document.eventform.action="<%=request.getContextPath()%>/marketing/delete_event.do?eventid="+eventid;
  ...
  function save(closeornew)
  {
    ...
    document.eventform.action="<%=request.getContextPath()%>/marketing/save_edit_event.do?TYPEOFREQUEST=EDIT&FromAttendee=NO&closeornew="+closeornew;
    document.eventform.target="_self";
    document.eventform.submit();
  }
  ...
  {
    data = individualLookupValues.idValue;
    document.eventform.Individualid.value = data;
    var eventid = document.eventform.eventid.value;
    document.eventform.action = "<%=request.getContextPath()%>/marketing/save_event_attendee.do?eventid=<%=request.getAttribute("eventid")%>&Individualid="+data;
    document.eventform.submit();
  }
  ...
  function addAttendees()
  {
    ...
    document.eventform.action="<bean:message key='label.url.root' />/marketing/save_edit_event.do.do?TYPEOFREQUEST=EDIT&FromAttendee=YES&closeornew=save";
    document.eventform.submit();
  }
  ...
  {
    var eventid = document.eventform.eventid.value;
    c_openWindow('/email/common_compose.do?type=EVENT&action=compose&eventid='+eventid,'', 720, 585, '');
  }

  function register()
  {
    var eventid = document.eventform.eventid.value;
    document.location.href = "<%=request.getContextPath()%>/marketing/register_event.do?eventid="+eventid;
    return false;
  }
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  <tr>
    ...
    <td class="sectionHeader" align="right">
      <c:if test="${requestScope.showRegisterButton}">
        <c:if test="${requestScope.hasAccepted}">
          <span class="labelBold"><bean:message key="label.marketing.unregisterevent"/>:</span>
          <html:button property="registerButton" styleClass="normalButton" onclick="return register()"><bean:message key="label.marketing.unregister"/>

          </html:button>
        </c:if>
        <c:if test="${not requestScope.hasAccepted}">
          <span class="labelBold"><bean:message key="label.marketing.register"/>:</span>
          <html:button property="registerButton" styleClass="normalButton" onclick="return register()"><bean:message key="label.marketing.register"/>
...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" class="formTable">
      ...
      <table width="100%" border="0" cellpadding="0">
        ...
        <tr>
          ...
          <td class="contentCell">
            ...
            <html:text property="moderatorname" styleClass="clickableInputBox" readonly="true" size="30" onclick="c_openPopup_FCI('Individual', document.eventForm.moderatorid.value)"/>
        ...
        <tr>
          <td class="labelCell"><bean:message key="label.marketing.messagetoallattendees"/>:</td>
          <td class="contentCell">
            <c:set var="disableButton" value=""/>
            <c:if test='${requestScope.valueList.parameters.totalRecords == "0"}'>
              <c:set var="disableButton" value='disabled'/>
            </c:if>
            <input name="compose" type="button" class="normalButton" value="<bean:message key='label.marketing.composemessage'/>" style="width:12em;"  <c:out value="${disableButton}" /> onclick="composeEmail();"  />
          </td>
        </tr>
      </table>
    </td>
        ...
        <tr>
          ...
          <td align="center" valign="middle">
            <table border="0" cellpadding="3" cellspacing="0">
              <tr>
                <td>
                  <app:cvbutton property="cmpattach" styleClass="normalButton" statuswindow="label.general.blank" tooltip="true" statuswindowarg= "label.general.blank" onclick="openAttachment()"><bean:message key="label.marketing.attachfile"/>
              ...
              <tr>
                <td>
                  <app:cvbutton property="changeLookup1" styleClass="normalButton"  onclick="downloadFile()"><bean:message key="label.marketing.download"/>
              ...
              <tr>
                <td>
                  <app:cvbutton property="removefile" styleClass="normalButton" statuswindow="label.general.blank" tooltip="true" statuswindowarg= "label.general.blank" onclick="removeAttachment()"><bean:message key="label.marketing.removefile"/>
 ...
 <tr>
    <td class="sectionHeader" colspan="2">
      ...
      <html:button property="cmpdelete" styleClass="normalButton" onclick="deleteevent()">
