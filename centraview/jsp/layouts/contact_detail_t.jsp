<%--
 * $RCSfile: contact_detail_t.jsp,v $    $Revision: 1.1.1.1 $  $Date: 2005/04/28 20:25:28 $ - $Author: mking_cv $
 * 
...
<html:html>
<head>
  <title><c:out value="${requestScope.dynamicTitle}" default="" /></title>
...
<table border="0" cellspacing="0" cellpadding="0" height="375">
  <tr valign="top"> 
    <td class="detailDarkBorder">
      <tiles:insert attribute="top_buttons" />
      <tiles:insert attribute="detail" />
      <tiles:insert attribute="bottom_buttons" />
    </td>
    <td>
      <iframe src="<html:rewrite page="/jsp/pages/contacts/contact_right_nav.jsp"/>?current=<c:out value="${requestScope.current}"/>&total=<c:out value="${requestScope.total}"/>&previous=<c:out value="${requestScope.previous}"/>&next=<c:out value="${requestScope.next}"/>&recordType=<c:out value="${requestScope.recordType}"/>" width="234" height="375" frameborder="0"></iframe>
    </td>
  </tr>
</table>
</div>
