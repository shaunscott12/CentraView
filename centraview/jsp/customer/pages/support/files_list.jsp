<%--
 * $RCSfile: files_list.jsp,v $    $Revision: 1.2 $  $Date: 2005/08/10 13:31:50 $ - $Author: mcallist $
 *
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tr>
    <td class="mainHeader"><bean:message key="label.customer.pages.support.files"/></td>
  </tr>
</table>
  ...
  <tr>
    <%
      ...
      for (int i = 0; i < vecColumns.size(); i++) {
        %><td nowrap class="listHeader"><span class="listHeader"><%=messages.getMessage( "FileList."+vecColumns.elementAt(i))%></span></td><%
      }
    %>
  </tr>
  <%
    ...
    if (DL.size() > 0) {
        ...
        while (enum.hasMoreElements()) {
          ...
          if (sm != null) {
            if (sm.getLinkEnabled()) {
              %><a href="<html:rewrite page="/customer/view_file.do"/>?fileID=<%=id%>" class="plainLink"><%=sm.getDisplayString()%></a>&nbsp;<%
            }else{
              %><%=sm.getDisplayString()%>&nbsp;<%
            }
          }else{
      ...
      %>
      <tr>
        <td class="tableRowOdd" colspan="6" align="center"><strong><bean:message key="label.customer.pages.support.norecordsfound"/>.</strong></td>
      </tr>
      <%
    }   // end if (DL.size() > 0)
  %>
