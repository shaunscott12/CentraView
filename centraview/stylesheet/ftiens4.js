...
{ 
  ...
  { 
    ...
    var j = this.desc;
    indexEmail =j.indexOf('±');
    if(indexEmail > 0){
    	j=j.substring(indexEmail+1,j.length);
    }
    else{
...
//    docW = docW + this.desc + "</a>"
  ...
  {
	  if (supportsDeferral) {
		  doc.write("<div class=\"plainText\" style=\"font-size:65%;\" id=domRoot></div>") //transition between regular flow HTML, and node-insert DOM DHTML
...
{ 
  ...
  { 
    ...
    if (this.hreference == "root"){
    	docW = docW + "<a class=\"fileNavOff\" "
    }
    else{
	if (USEFRAMES)
	  docW = docW + "<a href='" + this.hreference+ "' TARGET=\"_parent\"  class=\"fileNavOff\" "
	else
	  docW = docW + "<a href='" + this.hreference+ "' TARGET=_parent  class=\"fileNavOff\" "
    }
    ...
    docW = docW + "<a class=\"leftNavTitle\">" 
...
{ 
  ...
  if (USETEXTLINKS) 
    docW = docW + "<a  class=\"plainLink\" " + fullLink + " id=\"itemTextLink"+this.id+"\">" + this.desc + "</a>"
  else 
    docW = docW + this.desc
...
function blockStartHTML(idprefix) {
  ...
  if (browserVersion == 2) 
    docW = "<layer class=\"plainText\" "+ idParam + " top=" + doc.yPos + " visibility=show>"
  else if (browserVersion != 0)
    docW = "<div class=\"plainText\" " + idParam + " style='display:block; position:block;'>"
...
{ 
  ...
  if (folderObj.id!=foldersTree.id && PERSERVESTATE)
  {
    ...
    if (!folderObj.isOpen) //closing
    {
      ...
      //alert(currentOpen+folderObj.getID()+cookieCutter);
    }
  }
}
...
function findObj(id)
{
  var i=0;
  var nodeObj;
  if (typeof foldersTree.desc != "undefined") {
    nodeObj = indexOfEntries[i];
    for(i=0;i<nEntries&&indexOfEntries[i].desc!=id;i++) //may need optimization
      ;
    id = i
  }
  if (id >= nEntries)
  ...
  if (browserVersion == 1 || browserVersion == 3) {
    ...
    if (clickedDOMObj != null) {
        if (lastClicked != null) {
            ...
            prevClickedDOMObj.style.fontWeight= 'normal';
            //prevClickedDOMObj.style.fontSize= '1';
        }
        
        lastClickedColor    = clickedDOMObj.style.color;
        lastClickedBgColor  = clickedDOMObj.style.backgroundColor;
        clickedDOMObj.style.fontWeight='bold';
       // prevClickedDOMObj.style.fontSize= 1;
    }
  }
  lastClicked = nodeObj;
...
function setInitialLayout(str) {

	if(str != " "){
    SetCookie('highlightedTreeviewLink', str)

		SetCookie("clickedFolder", str+"^");
	}
...
function initializeDocument(value) 
{ 
  preLoadIcons();
