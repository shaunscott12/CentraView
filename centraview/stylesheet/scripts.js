...
function selectRow(element, isOdd)
{
	if (element.id != "headCheck") 
	{
		...
		if(element.id == "bottomCheck") 
		{ 
			cssNameCheck="contactTableCheckBottomSelected";
		}
		if(!isSelected)
		{
			if (isOdd) 
			{
				cssName = "contactTableRowOdd";
			} else {
				cssName = "contactTableRowEven";
			}
		...
		if(parentRow)
		{
			parentRow.className=cssName;
		}
		var parentColumn = element.parentNode;
		if(parentColumn)
		{
			parentColumn.className = cssNameCheck;
		}
	}
}
...
function showMainWindow(url) {
	...
	myWindow.opener.location.href=url;
...
function selectAll(depricated, single) 
{
  ...
  if (single) 
  {
    ...
    for (i=0;i<cbElements.length;i++) {
      if (cbElements[i].getAttribute("type")=="checkbox") {
        if (cbElements[i].id == "headCheck" && !depricated.checked)	cbElements[i].checked = false;
...
{
	window.open(page,"Centraview","height=200,width=400,status=yes,toolbar=yes,menubar=yes,location=yes");
}
...
{
	var url = "/centraview/additional_menu.do?UIParam=0&forward="+forward+params;
	if(isNewWin == 1)
	{
...
var lookupEntityID = "";
function fnSetLookupEntityID(lookupEntityID){
	this.lookupEntityID = lookupEntityID;
}

function fnGetLookupEntityID(){
...
//	formName="document."+formName
	...
	if (eval(formName+".dataPresent.value") <= 0)
	{
	  if (lookupName == "address"){
		...
		if (lookupName == "primary"){
		  window.opener.fnSetLookupEntityID(0);
	...
	if(flag2==0){ 
		...
		if (lookupName == "lookup_attendee"){
			var strCheck = "";
			i=0;
			while(i<document.IndividualLookupForm.radio.length)
			{
				if(document.IndividualLookupForm.radio[i].checked)
				{
					var value = window.document.IndividualLookupForm.radio[i].value;
					if (strCheck=="")
					{
						data = data + value;
						strCheck="checked";
  ...
  if (lookupName == "address"){
	...
	if (lookupName == "primary"){
	  window.opener.fnSetLookupEntityID(entityID);
		window.opener.setPrimary(name,idValue,midleName ,lastName,title);
	}
	if (lookupName == "lookup_attendee"){
			window.opener.setView(data);
		}
  ...
  // this function checks to see how many checkboxes are checked on
  // the List. If any number other than 1 are checked, then this
  // function will return false. Otherwise, this function returns
  // true when exactly one checkbox is checked.
  function onlyOneChecked()
  {
    var numberChecked = 0;
    // loop through each form element in the form
    for (i=0; i<document.listrenderer.elements.length; i++)
    {
      if (document.listrenderer.elements[i].type == "checkbox" )
      {
        // if the element is a checkbox, check to see if it's checked
        if ((document.listrenderer.elements[i].checked))
        {
          // if this checkbox is checked, then increment the 
          // number of checked boxes by 1.
          numberChecked++;
        }
      }
    } 
    ...
    if (numberChecked == 1){
      // if there is exactly one box checked, return true
      return true;
    }else{
      // 0 or more than 1 boxes are checked
      return false;
    }
  }   // end function onlyOneChecked()

//For custom tag, select box
function dontSelect(obj)
{
	...
	for(var i=0;i<len;i++)
	{
		if( document.forms[0].elements[i].name == obj.name )
		{
			...
			{			
				if(	document.forms[0].elements[j] != undefined )
				{
					document.forms[0].elements[i].focus();		
					break;
				}
				else if(document.forms[0].elements[j].type != "hidden" )
				{
...
//For custom tag, select box ends here
...
function goHelp(helpAction){
    helpWindow = window.open('/centraview/help/CentraView.htm', 'HelpIndex', "width=700,height=550,scrollbars=yes,resizable=yes");
}

function openWindows(page, width, height, windowOptions){
  if (windowOptions == ''){
    windowOptions = 'scrollbars=yes,resizable=yes';
  }

  window.open(page, '', "width="+width+",height="+height+","+windowOptions);
}
...
function openEntityDetail_FCI(entityIDurl)
{
	if (entityIDurl > 0)
	{
		openPopup('/centraview/ViewEntityDetail.do?rowId=' + entityIDurl + '&listId=');
	}
}

function openIndividualDetail_FCI(individualIDurl)
{
	if (individualIDurl > 0)
	{
		openPopup('/centraview/ViewIndividualDetail.do?rowId=' + individualIDurl + '&listId=');
	}
}